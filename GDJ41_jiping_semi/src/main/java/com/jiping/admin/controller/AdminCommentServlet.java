...
public class AdminCommentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<LectureComment> reportCommentList= new LectureService().reportCommentList(cPage,numPerPage);
			
		//pageBar 작성하기
		int totalData = new LectureService().selectReportCommetAllCount();
		...
		if(pageNo==1) {
			...
			pageBar="<a href='"+request.getContextPath()
					+"/admin/adminComment?cPage="+(pageNo-1)+"'>&laquo;</a>";
		}
		...
		while(!(pageNo>pageEnd||pageNo>totalPage)) {
			if(cPage==pageNo) {
				...
				pageBar+="<a href='"+request.getContextPath()
						+"/admin/adminComment?cPage="+pageNo+"'>"+pageNo+"</a>";
			}
			pageNo++;
		}
		if(pageNo>totalPage) {
			...
			pageBar+="<a href='"+request.getContextPath()
					+"/admin/adminComment?cPage="+pageNo+"'>&raquo</a>";
		}
